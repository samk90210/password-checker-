/* script.js
   Features:
   - Smooth light/dark toggle (soft dark)
   - YouTube lofi background (muted autoplay) + controls
   - Password checker + entropy + time-to-crack estimates
   - Password history (localStorage) with copy/export/clear
   - Generator modal + gamified full-screen password builder
...
// Config & constants
// -----------------------------
const YT_VIDEO_ID = "8b3fqIBrNW0"; // user-provided lofi
const MUSIC_PREF_KEY = "pw_music_prefs_v2";
const THEME_KEY = "pw_theme_v2";
const HISTORY_KEY = "pw_history_v2";

// guess rates (guesses per second)
const GUESSES = {
  "1e3": 1e3,      // online throttled attacker
  "1e9": 1e9,      // single GPU rig, approximate
  "1e12": 1e12     // very powerful attacker / clusters
};
...
// fallback audio (unused if YT works)
const fallbackAudio = $("#fallbackAudio");
...
function applyTheme(dark){
  ...
  // smooth transition: tune CSS transitions already set
...
playPauseBtn.addEventListener("click", () => {
  ...
  if (state === YT.PlayerState.PLAYING) {
    ...
    musicPrefs.playing = false;
  } else {
    ytPlayer.playVideo();
    playPauseBtn.textContent = "⏸ Pause";
    musicPrefs.playing = true;
  }
  saveMusicPrefs();
});
...
function estimateEntropy(password){
  ...
  // determine pool size from character classes present
  let pool = 0;
  if (/[a-z]/.test(password)) pool += 26;
  if (/[A-Z]/.test(password)) pool += 26;
  if (/[0-9]/.test(password)) pool += 10;
  if (/[\W_]/.test(password)) pool += 32; // rough special-char count
  // fallback: if pool still 0 (weird), assume 26
  if (pool === 0) pool = 26;
...
function timeToCrackSeconds(entropyBits, guessesPerSecond){
  // On average to find a random password: 2^entropy / 2 guesses? Conservative estimate: 2^(entropy-1) / guesses
  // We'll estimate expected tries = 2^(entropy - 1)
...
function evaluatePassword(password){
  ...
  let strengthLabel = "Requirements not met ❌";
  let color = "var(--danger)";
  if (!lengthOK) {
    strengthLabel = "Requirements not met ❌";
    color = "var(--danger)";
  } else if (lengthStrong && score === 5) {
    strengthLabel = "Strong ✅";
    color = "var(--good)";
  } else {
    strengthLabel = "Intermediate ⚠️";
    color = "var(--warn)";
  }

  const entropy = estimateEntropy(password);
  return { strengthLabel, color, score, requirements:{lengthOK,lengthStrong,hasUpper,hasLower,hasNumber,hasSpecial}, entropy };
}

function refreshUIFromPassword(){
  const pw = passwordInput.value || "";
  const res = evaluatePassword(pw);
  strengthText.textContent = `Strength: ${res.strengthLabel}`;
  strengthText.style.color = "";

  // progress width from score
  ...
  const t1 = timeToCrackSeconds(entropy, GUESSES["1e3"]);
  const t2 = timeToCrackSeconds(entropy, GUESSES["1e9"]);
  const t3 = timeToCrackSeconds(entropy, GUESSES["1e12"]);
...
copyBtn.addEventListener("click", async () => {
  try {
    ...
    alert("Unable to copy automatically. Select & copy manually.");
  }
});
...
$("#saveHistoryBtn").addEventListener("click", () => {
  ...
  addToHistory(pw);
  alert("Saved to local history (local only).");
});
...
function renderHistory(){
  ...
  if (!list.length) {
    historyList.innerHTML = "<div style='color:#777'>No saved passwords.</div>";
    return;
  }
  ...
  list.forEach(entry => {
    ...
    left.style.flex = "1";
    left.innerHTML = `<div style="font-weight:600">${entry.password}</div><div style="font-size:12px;color:#666">Entropy: ${entry.entropy} bits • ${new Date(entry.savedAt).toLocaleString()}</div>`;
    ...
    copyBtn.addEventListener("click", async ()=> {
      try { await navigator.clipboard.writeText(entry.password); copyBtn.textContent = "✅"; setTimeout(()=>copyBtn.textContent="Copy",900); } catch { alert("Copy failed"); }
    });

    const useBtn = document.createElement("button");
    useBtn.textContent = "Use";
    useBtn.className = "pill";
    ...
    delBtn.addEventListener("click", () => {
      const arr = loadHistory().filter(e => e.id !== entry.id);
      saveHistory(arr);
      renderHistory();
    });
...
clearHistoryBtn.addEventListener("click", ()=> {
  if (!confirm("Clear saved password history? This cannot be undone.")) return;
  saveHistory([]);
  renderHistory();
});
...
downloadHistoryBtn.addEventListener("click", ()=> {
  ...
  a.href = url;
  a.download = `password_history_${new Date().toISOString().slice(0,10)}.json`;
  a.click();
  URL.revokeObjectURL(url);
});

// quick export from settings button
...
const securitySteps = [
  ...
  { q: "Enter a 4-digit number that is NOT sequential like 1234 or all repeated like 1111", test: s => {
      ...
      for(let i=1;i<digits.length;i++){ if(digits[i] !== digits[i-1]+1) inc=false; if(digits[i] !== digits[i-1]-1) dec=false; }
      if(inc||dec) return false; return true;
    }, transform: s=>s },
...
function closeGamePanel(){
  ...
  gameAnswer.value = ""; gameMessage.textContent = "";
}
function showGameStep(){
  const s = securitySteps[gameIndex];
  gameQuestion.textContent = s.q;
...
submitAnswerBtn.addEventListener("click", ()=> {
  ...
  if (step.test(v)){
    ...
    if (gameIndex < securitySteps.length){
      ...
      refreshUIFromPassword();
      gameMessage.textContent = "🎉 Password built and filled!";
      setTimeout(()=> {
        closeGamePanel();
...
skipBtn.addEventListener("click", ()=> {
  ...
  else { passwordInput.value = builtPassword; refreshUIFromPassword(); closeGamePanel(); setActiveModeUI("password"); }
});

function flashGameMessage(text){
  gameMessage.classList.remove("ok");
  gameMessage.textContent = text;
  setTimeout(()=> { if (gameMessage.textContent === text) gameMessage.textContent = ""; }, 2200);
}
...
function animateGameStepTransition(nextFn){
  ...
  setTimeout(()=> { nextFn(); el.style.transform="translateX(0)"; el.style.opacity="1"; }, 220);
}
...
// -----------------------------
// tiny initializations & UX
// -----------------------------
passwordInput.addEventListener("input", refreshUIFromPassword);
refreshUIFromPassword();

(function revealMain(){
  mainCard.style.opacity = 0; mainCard.style.transform = "translateY(8px)";
  setTimeout(()=> { mainCard.style.transition = "transform .48s var(--ease), opacity .36s var(--ease)"; mainCard.style.opacity=1; mainCard.style.transform="translateY(0)"; }, 60);
})();
